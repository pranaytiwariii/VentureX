        ...
        <main className="flex-grow">
          <Card className="mb-8">
            <CardHeader className="flex flex-row items-center gap-4">
              <img src={projectData.logo} alt={`${projectData.name} logo`} className="w-16 h-16 rounded-full" />
              <div>
            ...
            <CardContent>
              <Tabs value={activeTab} onValueChange={setActiveTab}>
                  ...
                  <ul className="space-y-4">
                    {projectData.updates.map((update, index) => (
                      <li key={index} className="border-b pb-4 last:border-b-0">
                        ...
                        <p className="text-sm text-gray-500 mb-2">{new Date(update.date).toLocaleDateString()}</p>
                        <p>{update.content}</p>
                      </li>
                    ))}
                  </ul>
                ...
                <TabsContent value="discussions">
                  <p>Investor discussions will be implemented here. This could include a comment system or forum integration.</p>
                </TabsContent>
              </Tabs>
            </CardContent>
          </Card>
        ...
        <aside className="w-full md:w-80">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Exclusive NFTs for project supporters</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {projectData.nfts && projectData.nfts.length > 0 ? (
                  projectData.nfts.map((nft) => (
                    <div key={nft.id} className="flex items-center gap-4">
                      <img src={nft.image} alt={nft.name} className="w-16 h-16 rounded" />
                      <div className="flex-grow">
